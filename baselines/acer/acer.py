...
class Model(object):
    def __init__(self, policy, ob_space, ac_space, nenvs, nsteps, ent_coef, q_coef, gamma, max_grad_norm, lr,
        ...
        train_ob_placeholder = tf.compat.v1.placeholder(dtype=ob_space.dtype,
        ...
        adv = qret - v
        logf = tf.compat.v1.log(f_i + eps)
        gain_f = logf * tf.stop_gradient(adv * tf.minimum(c, rho_i))  # [nenvs * nsteps]
        loss_f = -tf.reduce_mean(gain_f)

        # Bias correction for the truncation
        adv_bc = (q - tf.reshape(v, [nenvs * nsteps, 1]))  # [nenvs * nsteps, nact]
        logf_bc = tf.compat.v1.log(f + eps)  # / (f_old + eps)
        ...
        if trust_region:
            ...
            norm_grads_q = tf.compat.v1.global_norm(grads_q)
            norm_grads_policy = tf.compat.v1.global_norm(grads_policy)
        else:
