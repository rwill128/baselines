    ...
    def _init(self, ob_space, ac_space, hid_size, num_hid_layers, gaussian_fixed_var=True):
        ...
        with tf.compat.v1.variable_scope('vf'):
            ...
            for i in range(num_hid_layers):
                last_out = tf.nn.tanh(tf.compat.v1.layers.dense(last_out, hid_size, name="fc%i" % (i + 1),
        ...
        with tf.compat.v1.variable_scope('pol'):
            ...
            if gaussian_fixed_var and isinstance(ac_space, gym.spaces.Box):
                mean = tf.compat.v1.layers.dense(last_out, pdtype.param_shape()[0] // 2, name='final',
                                       kernel_initializer=U.normc_initializer(0.01))
                logstd = tf.compat.v1.get_variable(name="logstd", shape=[1, pdtype.param_shape()[0] // 2],
                                                   initializer=tf.zeros_initializer())
                pdparam = tf.concat([mean, mean * 0.0 + logstd], axis=1)
            else:
                pdparam = tf.compat.v1.layers.dense(last_out, pdtype.param_shape()[0], name='final',
                                          kernel_initializer=U.normc_initializer(0.01))
